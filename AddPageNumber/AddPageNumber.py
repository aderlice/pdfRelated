...
createPagePdf(len(reader_base.pages) - base_page, tmp)
reader_nums = PdfReader(tmp)

i = base_page
for i in range(len(reader_base.pages)):
    if i - base_page in except_pages:
        continue
    reader_base.pages[i].merge_page(reader_nums.pages[i - base_page])
